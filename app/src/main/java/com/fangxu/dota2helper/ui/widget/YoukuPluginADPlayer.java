...
public class YoukuPluginADPlayer extends PluginOverlay implements DetailMessage {
...
//	private TextView mAdSkip;
	...
	private String mVideoBackground = null;

	public YoukuPluginADPlayer(YoukuBasePlayerManager basePlayerManager,
							   IMediaPlayerDelegate mediaPlayerDelegate) {
	...
	public YoukuPluginADPlayer(YoukuBasePlayerManager basePlayerManager,
							   IMediaPlayerDelegate mediaPlayerDelegate, String backgoundUrl) {
		super(basePlayerManager.getBaseActivity(), mediaPlayerDelegate);
		this.mediaPlayerDelegate = mediaPlayerDelegate;
		mBasePlayerManager = basePlayerManager;
		//mActivity = mBasePlayerManager.getBaseActivity();
...
//		if (Profile.PLANTFORM == Plantform.YOUKU) {
			containerView = mLayoutInflater.inflate(
					R.layout.youku_player_ad_youku, null);
//		} else {
//			containerView = mLayoutInflater.inflate(
//					R.layout.yp_player_ad_tudou, null);
//		}
		addView(containerView);
		ImageView blur = (ImageView)containerView.findViewById(R.id.iv_blur);
		if (mVideoBackground != null) {
			Glide.with(mContext).load(mVideoBackground).asBitmap().placeholder(com.fangxu.dota2helper.R.color.black).transform(new BlurTransformation(mContext, 20)).into(blur);
		}
